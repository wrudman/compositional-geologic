...
DEMO_STEPS = {
    2: {
        "title": TUTORIAL_SELECTION_TITLE,
        "definition": "",
        "context": "",
        "instruction": "",
...
def session_query_params():
    """Keep preview routing when replacing transient action parameters."""
    params = {
...
if not st.query_params.get("survey_instance"):
    next_params = session_query_params()
...
def mark_annotation_tutorial_completed(
    data: dict, completion_method: str = "guided_tutorial"
) -> None:
    summary = data.setdefault("tutorial_summary", {})
    completed_at = _ts()
...
def render_demo_direction_diagram(session):
    ...
    draw_tutorial_faces(draw, res_map, session.face_label_cache)
...
def begin_survey(
    data: dict, tutorial_completion_method: str = "guided_tutorial"
) -> None:
...
def clicked_selection_index(data: dict, action: str, params) -> int | None:
    """Find an existing selection so another click uses the removal path."""
    if action == "select_vertex":
        target = str(params.get("bridge_tgt", "none"))
        keys = [str(value) for value in data.get("selected_vertex_ids", [])]
    elif action == "select_region":
        target = int(params.get("bridge_face", "-1"))
        keys = [int(value) for value in data.get("selected_region_indices", [])]
    elif action == "select_angle":
        target = (str(params.get("bridge_tgt", "none")), int(params.get("bridge_face", "-1")))
        keys = [(str(item.get("vertex_id")), int(item.get("face_idx", -1)))
                for item in data.get("selected_angles", [])]
    elif action == "select_edge":
        target = frozenset((int(params.get("bridge_tail", "-1")), int(params.get("bridge_head", "-1"))))
        keys = [frozenset((int(item.get("tail_id", -1)), int(item.get("head_id", -1))))
                for item in data.get("selected_edges", [])]
    else:
        return None
    return keys.index(target) if target in keys else None
...
sess = data["session"]
if data.get("phase", "demo") == "demo":
    apply_tutorial_label_positions(sess.res_map, sess.face_label_cache)
...
st.markdown(
    ...
    <style>
        ...
        div[data-testid="stForm"]:has(#survey-answer-style-anchor) div[data-testid="stTextArea"] textarea {
            min-height:60px !important; height:60px !important;
            padding:0.45rem 0.75rem !important;
        }
        div[data-testid="stForm"]:has(#survey-answer-style-anchor) button {
            min-height:2.5rem !important; padding:0.4rem 0.75rem !important;
        }
        h1, h2, h3 {
...
if (
    ...
    if not UNION_TOOL_ENABLED and act in {
        ...
        save_session(data)
    clicked_index = clicked_selection_index(data, act, query_params)
    if clicked_index is not None:
        act = act.replace("select_", "remove_selected_", 1)
    ...
    if act == "select_angle":
        ...
        remove_index = clicked_index if clicked_index is not None else int(query_params.get("bridge_selection_index", "-1"))
        selected_angles = list(data.get("selected_angles", []))
...
if data.get("phase") == "demo":
    tutorial_step = data.get("demo_step", 2)
...
feedback = data.get("answer_feedback") or {}
feedback_for_current = (
    feedback.get("trial_index") == current_trial_index
    and feedback.get("question_id") == current_question["question_id"]
)
...
# Keep headings, instructions, feedback, and the diagram in the same
# left column. The Help column shares its row and needs no pixel offsets.
selection_intro = data.get("phase") == "demo" and data.get("demo_step") == 2
if show_drawing_pad:
    if data.get("phase") == "demo":
        ...
        render_survey_header(
            ...
            str(current_question["question_text"]),
        )
if show_drawing_pad:
    main_work_col, main_info_col = st.columns([8, 3], gap="small", vertical_alignment="top")

with main_work_col if show_drawing_pad else nullcontext():
    if data.get("phase") == "demo":
        demo_step = data.get("demo_step", 0)
        if demo_step == 0:
            ...
            # Selection instructions are rendered inside the diagram component.
            # Do not insert an empty Streamlit column row above that component.
            practice_copy_col = nullcontext() if demo_step == 2 else st.container()
            ...
            render_survey_header("Practice", "Determine whether the arrows move clockwise or counterclockwise.", practice=True)
            clockwise_work_col, clockwise_help_col = st.columns(
                [8, 3], gap="small", vertical_alignment="top"
            )
            with clockwise_work_col:
                clockwise_copy_col, clockwise_diagram_col = st.columns(
                    [3, 5], gap="small", vertical_alignment="top"
                )
            ...
            with clockwise_diagram_col:
                st.image(render_demo_direction_diagram(sess), width=DISPLAY_SIDE)
            with clockwise_help_col:
                st.markdown(
                    '<div class="survey-side-heading" data-section="Help" style="font-size:1.25rem; font-weight:600; '
                ...
                st.markdown(
                    '<div class="survey-side-heading" data-section="Quick actions" style="font-size:1.25rem; font-weight:600; '
                ...
                st.markdown(
                    '<div class="survey-side-heading" data-section="Sketch pad" style="font-size:1.25rem; font-weight:600; '
                ...
                st.markdown(
                    '<div class="survey-side-heading" data-section="Output" style="font-size:1.25rem; font-weight:600; '
                    'margin:0.8rem 0 0;">Output</div>',
                    unsafe_allow_html=True,
                )
                st.caption("(results will appear here)")
        elif demo_step == DEMO_FRAME_STEP:
            render_survey_header("Practice", "Review the frame and outside of the frame.", practice=True)
            frame_work_col, frame_help_col = st.columns(
                [8, 3], gap="small", vertical_alignment="top"
            )
            with frame_work_col:
                frame_copy_col, frame_diagram_col = st.columns(
                    [3, 5], gap="small", vertical_alignment="top"
                )
            with frame_copy_col:
                st.markdown(
            ...
            with frame_diagram_col:
                st.image(render_demo_frame_diagram(sess), width=DISPLAY_SIDE)
            with frame_help_col:
                st.markdown(
                    '<div class="survey-side-heading" data-section="Help" style="font-size:1.25rem; font-weight:600; '
                    'line-height:1.2; margin:0 0 0.25rem;">Help</div>',
    ...
    """
        ...
        with answer_col:
            if feedback_for_current:
                ...
                with st.form("answer_form", clear_on_submit=False, enter_to_submit=False):
...
if not show_drawing_pad:
    # The static direction/frame lessons already own their three-column layout.
...
with main_info_col:
    if show_drawing_pad:
        st.markdown(
            '<div class="survey-side-heading" data-section="Help" style="font-family:\'Source Sans 3\', \'Source Sans Pro\', sans-serif; '
...
bg_image = sess.render(base_renderer=draw_tutorial_faces if data.get("phase") == "demo" else None)
if data.get("phase") == "demo":
...
component_display_side = DISPLAY_SIDE
...
component_body_display = "grid" if data.get("phase") == "demo" else "flex"
...
component_control_column_width = "100%" if data.get("phase") == "demo" else f"{component_control_width}px"
component_body_gap = 16 if data.get("phase") == "demo" else 56
...
tool_box_guide_html = (
    '<div style="background:#eff6ff; color:#1e3a8a; border:1px solid #bfdbfe; '
    'border-radius:0.5rem; padding:0.55rem 0.7rem; margin:0.75rem 0 22px; font-size:0.9rem;">'
    + html.escape(TUTORIAL_BLUE_BOX_GUIDE) + '</div>'
    if data.get("phase") == "demo" and data.get("demo_step") == (3 if HIGHLIGHT_TOOL_ENABLED else 4)
    else ""
)
...
<head>
    <style>
        ...
        .selection-row {{ display:flex; align-items:center; justify-content:space-between; gap:8px; padding:5px 7px; margin-top:4px; border-radius:6px; color:#374151; font-size:14px; line-height:1.4; }}
        ...
        .tool-choice {{ background:#fff; color:#374151; border:1px solid #cbd5e1; padding:8px 7px; border-radius:5px; cursor:pointer; font-size:15px; font-weight:400; line-height:1.4; text-align:left; }}
...
<body>
    ...
    <script>
        ...
        function edgeLabelPosition(seg, nx, ny) {{
            // Keep a padded rectangle around each immutable region letter.
            let best = null;
            for (const side of [1, -1]) {{
                for (const t of [.5, .35, .65, .2, .8]) {{
                    for (const offset of [30, 42, 54]) {{
                        const x = seg.x1 + (seg.x2-seg.x1)*t + side*offset*nx;
                        const y = seg.y1 + (seg.y2-seg.y1)*t + side*offset*ny;
                        const hits = facesData.filter(f => Math.abs(x-f.cx)<32 && Math.abs(y-f.cy)<32).length;
                        const overflow = Math.max(0, 18-x, x-382) + Math.max(0, 18-y, y-382);
                        const score = hits*10000 + overflow*1000 + (side<0?100:0) + Math.abs(t-.5)*100 + offset;
        ...
        function updateHoverTarget(e, fromClick = false) {{
            ...
            if (!fromClick && selectedElement?.type === 'Edge' && activeCategory === 'draw') return;
            // Angle selections remain replaceable; other selected objects stay
            // locked until explicitly removed.
            if (!fromClick && selectedElement && !['Angle', 'Edge'].includes(toolMode)) return;
        ...
        interCanvas.addEventListener('mousemove', updateHoverTarget);

        interCanvas.addEventListener('click', function(e) {{
            // A rerender resets hover state even when the pointer does not move.
            updateHoverTarget(e, true);
            ...
            if (toolMode === "Edge" && lockedEdge && hoverEdge) {{
                const selectedIndex = selectedEdges.findIndex(edge =>
                    (Number(edge.tail_id) === Number(lockedEdge.tail_id)
                        && Number(edge.head_id) === Number(lockedEdge.head_id))
                    || (Number(edge.tail_id) === Number(lockedEdge.head_id)
                        && Number(edge.head_id) === Number(lockedEdge.tail_id))
                );
                if (selectedIndex >= 0) {{
                    // Remove immediately; never insert a transient row for OFF.
                    document.querySelector(
                        '.selection-remove[data-kind="edge"][data-id="' + selectedIndex + '"]'
                    )?.closest('.selection-row')?.remove();
                    ...
                    hoverEdge = null;
                    showEdgePanel(null);
                    document.getElementById('selectionSection').classList.toggle(
                        'hidden', totalSelectedInputs() === 0
                    );
                    redraw();
                    updateRunButtonStates();
                    dispatchAction('remove_selected_edge', {{ bridge_selection_index: selectedIndex }});
                    return;
                }}
        ...
        function strokeWithHalo(ctx, halo = 2) {{
            ctx.save();
        ...
        function redraw() {{
            ...
            for (const selectedId of selectedVertexIds) {{
                ...
                if (!v) continue;
                interCtx.beginPath();
                interCtx.arc(v.x, v.y, 9, 0, 2*Math.PI);
                interCtx.strokeStyle = 'rgba(0,255,204,1)';
                interCtx.lineWidth = 2;
                strokeWithHalo(interCtx, 1);
            ...
            for (let edgeIndex = 0; edgeIndex < selectedEdges.length; edgeIndex++) {{
                ...
                if (persistentEdge) {{
                    ...
                    if (segs.length) {{
                        ...
                        const {{x, y}} = edgeLabelPosition(seg, nx, ny);
                        interCtx.font = '600 15px -apple-system, BlinkMacSystemFont, sans-serif';
                        interCtx.lineWidth = 4;
                        interCtx.strokeStyle = 'white';
                        ...
                        interCtx.fillStyle = 'rgb(0,65,85)';
            ...
            const primaryColor = renderLock ? '#FF4B4B' : 'rgba(150,150,150,0.85)';
            const strokeWidth = renderLock ? 5 : 4;

            if (toolMode === "Vertex" && targetV && !targetV.is_obsolete) {{
                interCtx.beginPath();
                interCtx.arc(targetV.x, targetV.y, renderLock ? 9 : 15, 0, 2*Math.PI);
                interCtx.strokeStyle = primaryColor;
                interCtx.lineWidth = renderLock ? 2 : strokeWidth;
                if (renderLock) strokeWithHalo(interCtx, 1); else interCtx.stroke();
                ...
                if (idx !== -1) {{
                    ...
                    if (!renderLock) {{
                        ...
                        interCtx.strokeStyle = 'rgba(150,150,150,0.7)';
                        interCtx.lineWidth = strokeWidth;
                        if (renderLock) strokeWithHalo(interCtx); else interCtx.stroke();
                        interCtx.beginPath();
                        interCtx.moveTo(targetV.x, targetV.y);
                    ...
                    interCtx.strokeStyle = renderLock ? 'rgba(203,32,107,1)' : 'rgba(150,150,150,0.85)';
                    interCtx.lineWidth = 3;
                    if (renderLock) strokeWithHalo(interCtx); else interCtx.stroke();
                    if (renderLock) {{
                    ...
                    const edgeColor = targetEdge.is_hidden
                        ...
                        : (renderLock ? 'rgba(0,255,255,0.92)' : 'rgba(150,150,150,0.85)');
                    const edgeWidth = 6;
                    for (let seg of segs) {{
                        interCtx.beginPath();
                        interCtx.moveTo(seg.x1, seg.y1);
                    ...
                    if (renderLock && segs.length > 0) {{
                        ...
                        const {{x: labelX, y: labelY}} = edgeLabelPosition(labelSeg, nx, ny);
                        interCtx.font = '600 15px -apple-system, BlinkMacSystemFont, sans-serif';
                        interCtx.lineWidth = 4;
                        interCtx.strokeStyle = 'white';
                        interCtx.save();
